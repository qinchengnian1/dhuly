...
public class ZipStrUtil {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("原始的字符串长度为------->"+len0+" "+System.currentTimeMillis());

        String ys = compress(str);
        System.out.println("\n压缩后的字符串为----->" + ys+" "+System.currentTimeMillis());
        float len1=ys.length();
        System.out.println("压缩后的字符串长度为----->" + len1+" @"+System.currentTimeMillis());

        String jy = unCompress(ys);
        System.out.println("\n解压缩后的字符串为--->" + jy+" @"+System.currentTimeMillis());
