...
(function ($) {
    ...
    fn.createCalendar = function () {
        ...
        if (!this.opts.hideFooterButton) {
            ...
            html += '    <input style="display: inline-block;margin: 15px 0 0 0px;height: 30px;" class="itext" id="delstartDay" name="delstartDay" type="date">';
            html += '    <span class="white-space">-</span>';
            html += '    <input style="display: inline-block;margin: 15px 0 0 0px;height: 30px;" class="itext" id="delendDay" name="delendDay" type="date">';
            html += '    <button style="margin-top:0;" onclick="deletePriceAll(this)" class="btn bg-primary btn-deletes">删除</button>';
            html += '    <button style="margin-top:0;" class="btn bg-success btn-reset">重置</button>';
            html += '    <button style="margin-top:0;" class="btn bg-primary btn-confirm">确定</button>';
            html += '    <button style="margin-top:0;" class="btn bg-white btn-cancel">取消</button>';
            html += '    </div>';
        }
        html += '</div>';
    ...
    fn._createDaySetupInputGroup = function () {
        ...
        for (var i = 0; i < config.length; i++) {
            var val = config[i];
            html += '<li>';
            html += i == 4 || i == 5 ? "" : '<label>' + val.name + '</label>';
            html += i == 4 || i == 5 ? '<input name="' + val.key + '" type="hidden">' : '<input name="' + val.key + '" type="text">';
            html += '</li>';
        }
        return html;
    };
    ...
    fn.dayComplate = function () {
        ...
        if (arr && arr instanceof Array) {
            for (var i = 0; i < arr.length; i++) {
                var val = arr[i];
                html += i == 3 ? '<p style="overflow: hidden;white-space: nowrap;width: 25px;cursor:pointer;">' + val.name + '{' + val.key + '}</p>' : '<p>' + val.name + '{' + val.key + '}</p>'
            }
        }
        return html;
    };
    ...
    fn.handleClickEvent = function () {
        ...
        //======================================================================
        // 删除
        this.calendar.on('click', '.btn-deletes', function (e) {
        ...
        this.calendar.on('click', '.valid-hook', function (e) {
            e.stopPropagation();
            e.preventDefault();
            // 当天日期
        ...
        this.settingWindow.on('click', '.btn-confirm', function () {
            if ($('input[name="min_num"]').val() == '') {
                alert('请输入最小成团人数');
                return false;
            } else if ($('input[name="left_num"]').val() == '') {
                alert('请输入剩余人数');
                return false;
            } else if ($('input[name="adult_price"]').val() == '') {
                alert('请输入成人价');
                return false;
            } else if ($('input[name="child_price"]').val() == '') {
                alert('请输入儿童价');
                return false;
            } else {
        ...
        function _handeSetData(data) {
            ...
            if (!IS_ENABLE) {
                ...
                var setDateRange = [];
                setDateRange.push(data.startDay);
                if (setDateRange) {
                    setDateRangeArr = setDateRange;
                }
                if (data.weeks.length > 0) {
                    // 处理与周n设置的交集
                    var intersectionDate = me.handleSetWeekData2(data.weeks, setDateRangeArr, data.endDay, data.startDay);
                    setDateRangeArr = setDateRangeArr.concat(intersectionDate);
                } else {
                    // 日期范围与单日的交集
                    var daysArr = me.handleSetDaysData2(data.days, setDateRangeArr, data.endDay, data.startDay);
                    setDateRangeArr = setDateRangeArr.concat(daysArr);
                }
            }
...
//启用价格日历获取初始化日期范围数组
     ...
     * @param setDateRangeArr 设置的日期范围或初始的日期范围
     */
    fn.handleSetWeekData2 = function (week, setDateRangeArr, endDay, startDay) {
        var me = this;
        var arr = [];
        // 日期范围长度
        var len = setDateRangeArr.length
        var resArr = len <= 1 ? this._getInitDaysRange2(endDay, startDay) : setDateRangeArr;
        $.each(resArr, function (index, val) {
            var weekNum = me.dateToObject(val).getDay();
            // week为数组，不用join方法indexOf无效？
            if (week.join(',').indexOf(weekNum) > -1) {
    ...
    // 启用日期范围与单日的交集
    fn.handleSetDaysData2 = function (days, setDateRangeArr, endDay, startDay) {
        var result = [];
        var len = setDateRangeArr.length;
        var resArr = len <= 1 ? this._getInitDaysRange2(endDay, startDay) : setDateRangeArr;
        // days数组字符串
